...
for img in "$COMPONENT_DIR"/*.jpg; do
    if [ -f "$img" ]; then
        ...
        if [ -n "$VERIFICATION" ]; then
            ...
            # Extract counts for terminal display
            HALLUCINATIONS=$(echo "$VERIFICATION" | grep -i "hallucinations:" | grep -o '[0-9]\+' | head -1 || echo "0")
            UNVERIFIED=$(echo "$VERIFICATION" | grep -i "unverified" | grep -o '[0-9]\+' | head -1 || echo "0")
...
TOTAL_HALLUCINATIONS=$(grep -i "hallucinations:" "$OUTPUT_FILE" | grep -o '[0-9]\+' | awk '{sum+=$1} END {print sum}' || echo "0")
TOTAL_UNVERIFIED=$(grep -i "unverified" "$OUTPUT_FILE" | grep -o '[0-9]\+' | awk '{sum+=$1} END {print sum}' || echo "0")
