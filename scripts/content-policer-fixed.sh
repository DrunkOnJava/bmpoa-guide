...
# Create reference context summary (simplified)
...
You are ContentPolicer, verifying content accuracy for the BMPOA Community Guide against reference materials.

Reference files available:
- Blue Mountain Property Owners Association Guide1.pdf
- Blue Mountain Subdivision Emergency Evacuation.pdf
- BMPOA-entities.txt (verified names and organizations)
- BMPOA-Outline.md (official document structure)
- database/ folder with contacts, governance, services data

Analyze page PAGE_NUM for:
...
Provide JSON analysis:
{
  ...
  "contentItems": [
    {
      "type": "phone|email|name|date|fee|policy|claim",
      "content": "specific content",
      "status": "verified|unverified|hallucination|suspicious",
      "source": "reference if verified",
      "issue": "problem description",
      "recommendation": "how to fix"
    }
  ],
  "verificationScore": 0-100,
  "hallucinations": ["definite hallucinations"],
  "unverifiedClaims": ["claims needing verification"],
  "citationsNeeded": ["statements needing sources"],
  "overallAssessment": "reliability summary"
}
EOF
