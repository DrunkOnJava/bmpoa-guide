...
# Includes ContentPolicer monitoring
...
while true; do
    ...
    if [ -n "$LATEST_BUILD" ]; then
        ...
        if [ -f "$BUILD_DIR/content-verification-progress.txt" ]; then
        ...
        if [ -f "$BUILD_DIR/content-verification-complete.flag" ] && [ "$LAST_POLICER_COUNT" -gt 0 ]; then
    ...
    if pgrep -f "content-policer.sh" > /dev/null; then
        RUNNING_PROCESSES="${RUNNING_PROCESSES}🚨 "
    fi
